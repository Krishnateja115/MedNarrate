...
POSSIBLE_PATHS = [
    ...
    os.path.abspath(
        "C:/Users/manas/Downloads/MedNarrate-main/data/mrad_full/MRAD/Unified_Medical_Dataset.csv"
    ),
]
...
class MRADDatasetLoader:
    ...
    def load_and_preprocess(self) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        df = df.drop_duplicates(subset=["Patient_ID", "Report_Text"]).reset_index(
            drop=True
        )
        ...
        logger.info(
            f"MRAD Dataset Loaded. Raw: {raw_count}, Usable: {usable_count}, Patients: {patient_count}"
        )
        return df, summary

    @staticmethod
    def patient_level_split(
        df: pd.DataFrame, random_seed: int = 42
        ...
        overlap_tt = set(train_df["Patient_ID"]).intersection(
        ...
        if overlap_tv or overlap_tt or overlap_vt:
            raise ValueError(
                f"CRITICAL: Patient data leakage detected! Overlap: TV={len(overlap_tv)}, TT={len(overlap_tt)}, VT={len(overlap_vt)}"
            )

        logger.info(
            f"Patient-level split completed. Train: {len(train_df)} ({len(train_pts)} pts), Val: {len(val_df)} ({len(val_pts)} pts), Test: {len(test_df)} ({len(test_pts)} pts)"
        )
