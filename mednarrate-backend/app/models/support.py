...
class SupportTicket(Base):
    ...
    related_report_id = Column(String(36), ForeignKey("reports.id", ondelete="SET NULL"), nullable=True)
    related_request_id = Column(String(100), nullable=True)
    related_incident_id = Column(String(36), ForeignKey("incidents.id", ondelete="SET NULL"), nullable=True)
...
class SupportTicketEvent(Base):
    ...
    event_type = Column(String(50), nullable=False) # e.g. "status_change", "diagnostic_snapshot", "escalation"
    content = Column(Text, nullable=False)
