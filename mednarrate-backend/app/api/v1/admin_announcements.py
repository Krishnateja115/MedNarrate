...
class AnnouncementCreate(BaseModel):
    ...
    audience: str = Field(default="all", example="patients") # all, patients, doctors, caregivers
    language: str = Field(default="en", example="en")
    start_time: datetime
    end_time: datetime
    status: str = Field(default="published", example="published") # draft, scheduled, published, expired
...
async def create_announcement(
    ...
    return {"message": "Announcement created successfully", "announcement": {"id": str(ann.id), "title": ann.title}}
...
async def update_announcement(
    ...
    if payload.start_time is not None:
        ann.start_time = payload.start_time.replace(tzinfo=None) if payload.start_time.tzinfo else payload.start_time
    if payload.end_time is not None:
        ann.end_time = payload.end_time.replace(tzinfo=None) if payload.end_time.tzinfo else payload.end_time
    ...
    return {"message": "Announcement updated successfully", "announcement": {"id": str(ann.id), "title": ann.title}}
