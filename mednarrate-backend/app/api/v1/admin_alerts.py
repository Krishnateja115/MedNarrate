...
async def get_admin_alerts(
    ...
    inc_stmt = select(Incident).where(Incident.status.in_(["open", "investigating", "identified"]))
    ...
    failed_rep_cnt = (await db.execute(select(func.count(Report.id)).where(Report.uploaded_at >= twenty_four_hours_ago, Report.processing_status.in_(["failed", "error"])))).scalar_one_or_none() or 0
    if failed_rep_cnt > 0:
    ...
    urgent_tkt_cnt = (await db.execute(select(func.count(SupportTicket.id)).where(SupportTicket.status == "open", SupportTicket.priority.in_(["p1", "p2", "urgent", "high"])))).scalar_one_or_none() or 0
    if urgent_tkt_cnt > 0:
    ...
    ai_errors_cnt = (await db.execute(select(func.count(LLMDiagnosticEvent.id)).where(LLMDiagnosticEvent.timestamp >= twenty_four_hours_ago, LLMDiagnosticEvent.status != "success"))).scalar_one_or_none() or 0
    if ai_errors_cnt > 0:
...
async def acknowledge_alert(
    ...
    return {"status": "ok", "message": f"Alert '{alert_id}' marked as acknowledged", "alert_id": alert_id}
