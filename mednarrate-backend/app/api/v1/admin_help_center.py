...
from app.models.help_center import HelpArticle, ArticleStatus
...
async def create_article(
    ...
    exists = (await db.execute(select(HelpArticle).where(HelpArticle.slug == payload.slug))).scalars().first()
    if exists:
...
async def update_article(
    ...
    if payload.title is not None: article.title = payload.title
    if payload.slug is not None: article.slug = payload.slug
    if payload.category is not None: article.category = payload.category
    if payload.content is not None: article.content = payload.content
    if payload.status is not None: article.status = payload.status
