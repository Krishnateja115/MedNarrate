...
async def list_privacy_requests(
    ...
    items = []
    for r in reqs:
        u_info = users_map.get(
            str(r.user_id), {"email": "Unknown User", "full_name": None}
        )
        items.append(
            {
                "id": str(r.id),
...
async def update_privacy_request_status(
    ...
    res = await db.execute(
        select(PrivacyDataRequest).where(PrivacyDataRequest.id == r_uuid)
    )
...
async def get_sensitive_access_history(
    ...
    stmt = (
        ...
        .where(AdminAuditLog.sensitive_access_flag is True)
