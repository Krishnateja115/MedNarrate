...
async def list_admins(
    ...
    stmt = select(User).where(User.role == UserRole.admin).order_by(desc(User.created_at))
...
async def create_admin(
    req: AdminCreateReq,
    request: Request,
    admin_ctx: AdminContext = Depends(require_any_permission(["admins.manage", "super_admin"])),
    db: AsyncSession = Depends(get_db)
):
    ...
    if not admin_ctx.is_super_admin:
        for r_id in req.role_ids:
            try:
                ...
                raise HTTPException(status_code=400, detail=f"Invalid role ID format: {r_id}")
    ...
    for r_id in req.role_ids:
        try:
            role_uuid = uuid.UUID(r_id)
            assign = AdminRoleAssignment(user_id=new_admin.id, role_id=role_uuid, assigned_by_id=admin_ctx.user.id)
            db.add(assign)
        except ValueError:
            pass
...
async def deactivate_admin(
    ...
    return {"status": "ok", "message": f"Admin {target_admin.email} deactivated and sessions revoked."}
...
async def assign_admin_roles(
    ...
    # Re-assign roles
    await db.execute(delete(AdminRoleAssignment).where(AdminRoleAssignment.user_id == admin_uuid))
    for r_id in req.role_ids:
        try:
            role_uuid = uuid.UUID(r_id)
            assign = AdminRoleAssignment(user_id=admin_uuid, role_id=role_uuid, assigned_by_id=admin_ctx.user.id)
            db.add(assign)
        except ValueError:
            pass
