...
async def get_report_diagnostics(
    ...
    limit = clamp_limit(limit)
    stmt = select(Report, ReportAnalysis).outerjoin(
        ...
        items.append(
            {
                ...
                else ("failed" if analysis and analysis.error_reason else "pending"),
                ...
                "processing_time": (
                    analysis.processed_at - report.uploaded_at
                ).total_seconds()
                if analysis and analysis.processed_at
                else None,
            }
...
async def get_report_diagnostic_snapshot(
    ...
    stmt_job = (
        select(JobExecution)
        .where(JobExecution.request_id == str(report_id))
    ...
    return {
        ...
        "llm_provider": analysis.llm_provider if analysis else None,
        "llm_model": analysis.llm_model if analysis else None,
        "processing_time": (analysis.processed_at - report.uploaded_at).total_seconds()
        if analysis and analysis.processed_at
        else None,
    }
