...
async def global_admin_search(
    request: Request,
    q: str = Query(..., min_length=1, description="Search term across users, reports, tickets, incidents, audit logs"),
    ...
    if admin_ctx.has_permission("reports.view") or admin_ctx.has_permission("admin"):
        ...
        for r in reports:
            p_status = r.processing_status.value if hasattr(r.processing_status, 'value') else str(r.processing_status)
            r_type = r.report_type.value if hasattr(r.report_type, 'value') else str(r.report_type)
            results.append({
                "type": "report",
                "id": str(r.id),
    ...
    if admin_ctx.has_permission("system.view") or admin_ctx.has_permission("admin"):
        ...
        for inc in incidents:
            inc_status = inc.status.value if hasattr(inc.status, 'value') else str(inc.status)
            results.append({
                "type": "incident",
                "id": str(inc.id),
                "title": inc.title,
