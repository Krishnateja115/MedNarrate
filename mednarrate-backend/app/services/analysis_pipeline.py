...
async def generate_with_timeout(prompt: str, timeout: int = 25):
    """Executes single LLM call with a short request timeout without uncontrolled retry storms."""
    return await asyncio.wait_for(generate(prompt, timeout=timeout), timeout=timeout)
...
async def run_analysis(report_id: uuid.UUID, db: AsyncSession = None):
    ...
    logger.info(f"[STAGE:START] Pipeline analysis started for report {report_id}")
    ...
    try:
        ...
        if not report:
            logger.error(f"[STAGE:ERROR] Report {report_id} not found in database.")
            return
        ...
        logger.info(f"[STAGE:EXTRACTION] Processing file: {report.file_path} ({report.file_type.value})")
        ...
        logger.info(f"[STAGE:EXTRACTION:SUCCESS] Extracted {len(cleaned_text)} characters.")
        ...
        # 2. Run NER -> entities
        logger.info(f"[STAGE:NLP] Running biomedical NER pipeline...")
        entities = []
        try:
            ...
            for ent in entities_raw:
                ...
                try:
                    ...
                    logger.warning(f"[STAGE:NLP:WARN] Discarding invalid entity: {ve}")
            logger.info(f"[STAGE:NLP:SUCCESS] Identified {len(entities)} entities.")
        except Exception as ner_e:
            logger.warning(f"[STAGE:NLP:FAIL] NER pipeline exception (continuing lab extraction): {ner_e}")

        # 3. Extract Structured Lab Values & Abnormal Findings
        logger.info(f"[STAGE:LAB_EXTRACT] Extracting structured lab parameters...")
            ...
            try:
                ...
                if valid_lab["flag"] != "normal":
                    try:
                        ...
                        logger.warning(f"[STAGE:LAB_EXTRACT:WARN] Discarding invalid abnormal finding: {ve2}")
            except ValidationError as ve:
                logger.warning(f"[STAGE:LAB_EXTRACT:WARN] Discarding invalid lab value: {ve}")
                
        logger.info(f"[STAGE:LAB_EXTRACT:SUCCESS] Extracted {len(structured_lab_values)} lab values ({len(abnormal_findings)} abnormal).")

        # 4. RAG Processing
        logger.info(f"[STAGE:RAG] Processing report chunks for RAG index...")
        try:
            await process_report_for_rag(report.id, cleaned_text, db)
            logger.info(f"[STAGE:RAG:SUCCESS] Report processed for RAG.")
        except Exception as rag_e:
            logger.warning(f"[STAGE:RAG:FAIL] RAG indexing failed: {rag_e}")

        # 5. User Role & Prompt Preparation
        ...
        # 6. LLM Generation
        logger.info(f"[STAGE:LLM] Generating summaries via configured LLM...")
        clinician_summary = await generate_with_timeout(clinician_prompt)
        patient_summary = await generate_with_timeout(patient_prompt)
        logger.info(f"[STAGE:LLM:SUCCESS] Generated clinician and patient summaries.")
