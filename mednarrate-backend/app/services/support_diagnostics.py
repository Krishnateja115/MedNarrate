        ...
        if report:
            ...
            if analysis:
                snapshot["analysis_status"] = (
                    analysis.verification_status or "unverified"
                )
                snapshot["failure_category"] = (
                    analysis.failure_category or "not_captured"
                )
                ...
                snapshot["llm_telemetry"] = {
                    **(snapshot["llm_telemetry"] or {}),
                    ...
                    "fallback_used": getattr(
                        llm_event, "fallback_used", "not_captured"
                    ),
            ...
            if recent_job:
                snapshot["recent_job"] = {
                    ...
                    if recent_job.started_at
                    else None,
                    "finished_at": recent_job.finished_at.isoformat()
                    if recent_job.finished_at
                    else None,
    ...
    if ticket.user_id:
        ...
        if recent_notification:
            snapshot["recent_notification"] = {
                ...
                "type": getattr(
                    recent_notification, "notification_type", "not_captured"
                ),
                "status": recent_notification.status,
                "sent_at": recent_notification.sent_at.isoformat()
                if recent_notification.sent_at
                else None,
