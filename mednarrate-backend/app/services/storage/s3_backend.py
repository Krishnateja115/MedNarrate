...
class S3Backend(StorageBackend):
    ...
    async def download_file(self, filename: str) -> bytes:
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to download from S3: {str(e)}"
            )
    ...
    async def delete_file(self, filename: str) -> bool:
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to delete from S3: {str(e)}"
            )
    ...
    async def file_exists(self, filename: str) -> bool:
        try:
            ...
            if error_code == "404":
                return False
            raise HTTPException(
                status_code=500, detail=f"Failed to check file in S3: {str(e)}"
            )
