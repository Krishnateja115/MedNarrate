    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            stmt = select(MedicationSchedule).where(
                MedicationSchedule.is_active is True
            )
                ...
                for reminder_time in schedule.times_of_day or []:
                    try:
                        ...
                        if h == current_hour and m == current_minute:
                            ...
                            token_stmt = select(PushToken).where(
                                PushToken.user_id == schedule.user_id
                            )
                        ...
                        logger.error(
                            f"Invalid reminder time format for schedule {schedule.id}: {reminder_time} - {e}"
                        )
...
def start_scheduler():
    scheduler.add_job(
        check_medication_reminders, "cron", minute="*", id="medication_reminders"
    )
    scheduler.start()
