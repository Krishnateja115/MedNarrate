...
def resolve_physical_path(file_path: str) -> str:
    ...
    if not file_path or not str(file_path).strip():
        raise ExtractionFileNotFoundError(
            "File path is empty.", failure_category="FILE_NOT_FOUND"
        )
    ...
    if ".." in parts:
        raise ExtractionFileNotFoundError(
            "Path traversal attempt detected.", failure_category="FILE_NOT_FOUND"
        )
    ...
    if not is_under_upload and not (
        os.path.isabs(file_path) and os.path.exists(file_path)
    ):
        raise ExtractionFileNotFoundError(
            f"Resolved path '{resolved}' is outside upload directory.",
            failure_category="FILE_NOT_FOUND",
        )
    ...
    if not os.path.exists(resolved) or not os.path.isfile(resolved):
        raise ExtractionFileNotFoundError(
            f"File not found on server disk: {file_path}",
            failure_category="FILE_NOT_FOUND",
        )

    if not os.access(resolved, os.R_OK):
        raise UnreadablePDFError(
            f"File is not readable due to permission restrictions: {file_path}",
            failure_category="PDF_EXTRACTION_ERROR",
        )

    if os.path.getsize(resolved) == 0:
        raise EmptyFileError(
            f"File is empty (zero bytes): {file_path}", failure_category="EMPTY_FILE"
        )
...
def run_ocr_on_image(img: Image.Image) -> tuple[str, str]:
    ...
    try:
        ...
        reader = easyocr.Reader(
            ["en"], gpu=False, verbose=False, model_storage_directory=model_dir
        )
    ...
    if (
        not tesseract_cmd
        and "easyocr" not in sys.modules
        and not locals().get("best_text")
        and not locals().get("best_easy_text")
    ):
    ...
    if file_type_lower in ["pdf", "application/pdf"]:
        try:
            ...
            raise CorruptPDFError(
                "Could not extract readable medical text: corrupt or invalid PDF document.",
                failure_category="CORRUPT_PDF",
            ) from open_err
        ...
        try:
            ...
            raise ImageDecodeError(
                "Unable to read this image file. The file may be corrupt or formatted improperly.",
                failure_category="IMAGE_DECODE_ERROR",
            ) from img_err
        ...
        raise UnsupportedFileTypeError(
            f"Unsupported file type: {file_type}", failure_category="FILE_INVALID"
        )

    # Validate extracted text against quality thresholds
    is_img = file_type_lower in [
        "image",
        "jpg",
        "jpeg",
        "png",
