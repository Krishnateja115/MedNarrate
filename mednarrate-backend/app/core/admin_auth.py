...
class AdminContext:
    def __init__(self, user: User, permissions: List[str]):
        ...
        self.is_super_admin = (
            "super_admin" in self.permissions or "Super Admin" in self.permissions
        )  # Simplified check
...
def require_active_break_glass(resource_type: str) -> Callable:
    ...
    async def _check(
        ...
        if (
            not admin_ctx.has_permission("reports.sensitive_view")
            and not admin_ctx.has_permission("chat.sensitive_view")
            and not admin_ctx.is_super_admin
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
