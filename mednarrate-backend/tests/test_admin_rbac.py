...
@pytest.mark.asyncio
async def test_support_admin_can_access_health(
    client: AsyncClient, support_admin: User, db_session: AsyncSession
):
    from app.core.security import create_access_token
    ...
    stmt = select(AdminAuditLog).where(
        AdminAuditLog.actor_admin_id == support_admin.id,
        AdminAuditLog.action == "ADMIN_HEALTH_CHECK",
    )
...
async def test_support_admin_can_access_kb(client: AsyncClient, support_admin: User):
    ...
    resp = await client.get(
        "/api/v1/admin/kb-stats", headers={"Authorization": f"Bearer {token}"}
    )
    assert resp.status_code == 200
...
@pytest.mark.asyncio
async def test_support_admin_denied_privileged_operation(
    client: AsyncClient, support_admin: User
):
    ...
    resp = await client.get(
        "/api/v1/admin/llm-status", headers={"Authorization": f"Bearer {token}"}
    )
    assert resp.status_code == 403
