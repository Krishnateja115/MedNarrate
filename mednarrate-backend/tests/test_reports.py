...
async def test_upload_wrong_filetype(client: AsyncClient, auth_headers):
    ...
    data = {"title": "Text Report", "report_date": "2024-01-01", "report_type": "other"}
...
async def test_patch_report_favourite(client: AsyncClient, auth_headers):
    ...
    assert patch_resp.json()["is_favourite"] is True
...
async def test_other_user_cannot_access(
    ...
    get_resp = await client.get(
        f"/api/v1/reports/{report_id}", headers=other_auth_headers
    )
...
async def test_export_csv(client: AsyncClient, auth_headers):
    file_content = b"%PDF-1.4 dummy pdf content"
    files = {"file": ("test.pdf", file_content, "application/pdf")}
    data = {"title": "Export Test", "report_date": "2024-01-01", "report_type": "blood"}
    ...
    export_resp = await client.get(
        f"/api/v1/reports/{report_id}/export/csv", headers=auth_headers
    )
    assert export_resp.status_code == 404
    assert (
        export_resp.json()["detail"]
        == "No structured lab data available for this report"
    )
