...
async def release_super_admin(db_session: AsyncSession):
    ...
    if not sa_role:
        sa_role = AdminRole(id=uuid.uuid4(), name="Super Admin", description="Full Governance Super Admin")
        db_session.add(sa_role)
        await db_session.flush()

    assignment = AdminRoleAssignment(id=uuid.uuid4(), user_id=user.id, role_id=sa_role.id)
...
@pytest.mark.asyncio
async def test_analytics_endpoint_success(client: AsyncClient, release_super_admin: dict):
...
@pytest.mark.asyncio
async def test_alerts_endpoint_and_acknowledge(client: AsyncClient, release_super_admin: dict):
    headers = release_super_admin["headers"]
    # 1. Fetch alerts
    ...
    # 2. Acknowledge alert
    ack_res = await client.post("/api/v1/admin/alerts/test_alert_id/acknowledge", headers=headers)
