...
async def test_llm_provider_gemini_valid_mocked(monkeypatch):
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        ...
        mock_resp.json.return_value = {
            "candidates": [
                {"content": {"parts": [{"text": "Mocked Gemini Response Text"}]}}
            ]
        }
...
async def test_llm_provider_gemini_invalid_placeholder_key(monkeypatch):
    ...
    with pytest.raises(
        LLMConfigurationError, match="Gemini API key is not configured or is invalid"
    ):
        await generate("Test prompt")
...
async def test_live_gemini_generation_if_credentials_available():
    ...
    if not key or not key.strip() or key in ["your_gemini_api_key_here", "placeholder"]:
        pytest.skip(
            "No live GEMINI_API_KEY available in environment. Live generation test skipped."
        )
...
def test_prompt_injection_resistance():
    ...
    assert formatted_prompt.find(
        "Ignore previous instructions"
    ) > formatted_prompt.find("clinician")
