...
@pytest.mark.asyncio
async def test_diagnostic_snapshot_omits_sensitive_data(
    db_session: AsyncSession, sample_ticket_with_report: SupportTicket
):
    ...
    assert snapshot["related_report"]["id"] == str(
        sample_ticket_with_report.related_report_id
    )
...
@pytest.mark.asyncio
async def test_diagnostic_snapshot_determines_actions(
    db_session: AsyncSession, sample_ticket_with_report: SupportTicket
):
    ...
    assert any(
        a["reason"]
        == "Extraction failure: extraction_failed. Full reprocess may resolve."
        for a in actions
    )
