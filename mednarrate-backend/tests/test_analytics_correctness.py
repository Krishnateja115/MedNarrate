...
@pytest.mark.asyncio
async def test_analytics_avoids_fabricated_data(client: AsyncClient, analytics_admin: User):
    ...
    resp = await client.get("/api/v1/admin/analytics?timeframe=7d", headers={"Authorization": f"Bearer {token}"})
