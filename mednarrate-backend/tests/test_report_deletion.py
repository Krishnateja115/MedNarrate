...
@pytest.mark.asyncio
async def test_delete_processed_report_with_child_records(client: AsyncClient, token_headers: dict, db_session: AsyncSession):
    ...
    upload_res = await client.post(
        ...
        files={"file": ("processed.pdf", b"%PDF-1.4 processed test content", "application/pdf")}
    )
    assert upload_res.status_code == 201
    ...
    analysis = ReportAnalysis(
        ...
        structured_lab_values=[{"test_name": "Hemoglobin", "value": 13.5, "unit": "g/dL"}]
    )
    ...
    translation = AnalysisTranslation(
        ...
        language="hi",
        patient_summary="मरीज़ का सारांश"
    )
