...
async def admin_approver(db_session: AsyncSession):
    ...
    stmt = select(AdminPermission).where(
        AdminPermission.name == "approve_sensitive_access"
    )
    perm = (await db_session.execute(stmt)).scalars().first()
    if not perm:
...
@pytest.mark.asyncio
async def test_breakglass_request_lifecycle(
    client: AsyncClient,
    admin_requester: User,
    admin_approver: User,
...
@pytest.mark.asyncio
async def test_breakglass_self_approval_blocked(
    client: AsyncClient, admin_requester: User, db_session: AsyncSession
):
    ...
    role_assignment = (
        await db_session.execute(
            select(AdminRoleAssignment).where(
                AdminRoleAssignment.user_id == admin_requester.id
            )
        )
    ).scalar_one()
    perm = (
    ...
    if not perm:
        ...
        await db_session.flush()
    db_session.add(
        AdminRolePermission(role_id=role_assignment.role_id, permission_id=perm.id)
    )
    await db_session.commit()
...
@pytest.mark.asyncio
async def test_sensitive_access_denied_without_grant(
    client: AsyncClient, admin_requester: User, test_report
):
...
@pytest.mark.asyncio
async def test_sensitive_access_allowed_with_grant(
    client: AsyncClient,
    admin_requester: User,
    admin_approver: User,
    test_report,
...
@pytest.mark.asyncio
async def test_sensitive_access_denied_with_expired_grant(
    client: AsyncClient,
    admin_requester: User,
    admin_approver: User,
    test_report,
...
@pytest.mark.asyncio
async def test_audit_log_atomicity(
    client: AsyncClient, admin_requester: User, db_session: AsyncSession
):
