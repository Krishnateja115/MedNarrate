...
def upgrade() -> None:
    ...
    with op.batch_alter_table('sensitive_access_grants', schema=None) as batch_op:
        ...
        batch_op.alter_column('expires_at',
               ...
               nullable=True)
        batch_op.create_index(batch_op.f('ix_sensitive_access_grants_status'), ['status'], unique=False)
        batch_op.create_foreign_key('fk_sensitive_access_grants_revoker_id', 'users', ['revoker_id'], ['id'], ondelete='SET NULL')
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('sensitive_access_grants', schema=None) as batch_op:
        batch_op.drop_constraint('fk_sensitive_access_grants_revoker_id', type_='foreignkey')
