...
models_dir = (
    "/Users/harsha/Downloads/Sem 5/NLP/mednarrate/mednarrate-backend/app/models"
)
...
def process_file(filepath):
    ...
    if "PickleType" in original and "JSON" not in content[: content.find("class ")]:
        content = re.sub(
            r"(from sqlalchemy .*?)(PickleType)(.*?)", r"\1JSON\3", content
        )
    ...
    content = re.sub(
        r"DateTime(?!\(timezone=True\))(?:\(\))?", "DateTime(timezone=True)", content
    )
    ...
    if (
        "Text" in content
        and "Text" not in original[: original.find("class ")]
        and "Text" not in content[: content.find("class ")]
    ):
        # crude but effective
