...
for pdf_path in pdf_paths:
    ...
    expected_present = {
        "haemoglobin", "hemoglobin", "total leucocyte count", "neutrophils",
        "lymphocytes", "eosinophils", "monocytes", "basophils",
        "rbc count", "mcv", "mch", "mchc", "hct", "rdw-cv", "rdw-sd",
        "platelet count", "platelets", "mpv",
    }
    found_names = {r["original_name"].strip().lower() for r in results}
    matched = {name for name in expected_present if any(name in fn or fn in name for fn in found_names)}
    ...
    assert len(results) >= 18, f"FAIL: only {len(results)} lab values extracted — expected ~20+."
    assert len(missing) <= 2, f"FAIL: too many expected real lab tests missing: {missing}"
