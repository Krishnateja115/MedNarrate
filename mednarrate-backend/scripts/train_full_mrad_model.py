...
def main():
    ...
    logger.info("Performing patient-level GroupKFold split (80% train, 10% val, 10% test)...")
    ...
    for cls_name, cls_metrics in metrics['per_class_report'].items():
        if isinstance(cls_metrics, dict):
            logger.info(f"  {cls_name}: Precision={cls_metrics['precision']:.4f}, Recall={cls_metrics['recall']:.4f}, F1={cls_metrics['f1-score']:.4f}")
