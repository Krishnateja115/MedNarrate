...
def test_happy_path(report_file_path, report_title):
    ...
    process_resp = requests.post(
        f"{BASE_URL}/reports/{report_id}/process", headers=headers
    )
    ...
    while status in ["uploaded", "processing"] and retries > 0:
        ...
        status_resp = requests.get(
            f"{BASE_URL}/reports/{report_id}/status", headers=headers
        )
    ...
    analysis_resp = requests.get(
        f"{BASE_URL}/reports/{report_id}/analysis", headers=headers
    )
    ...
    compare_resp = requests.get(
        f"{BASE_URL}/reports/{report_id}/compare-previous", headers=headers
    )
    ...
    chat_resp = requests.post(
        ...
        headers=headers,
        json={"report_id": report_id, "title": "Discussion about report"},
    )
    ...
    msg_resp = requests.post(
        ...
        headers=headers,
        json={"content": "What is my fasting glucose?"},
    )
